...
Section~\ref{sec:carrier} then shows that for uniform $\bm{\omega}$ the
rotating-frame reduction removes it exactly, in discrete time as well as
continuous.
...
variant requires the $\bm{\omega}=\bm{0}$ case or the rotating-frame
reduction of Section~\ref{sec:carrier}).  Then at a projected fixed point,
\[
...
\begin{remark}[What $\omega_{\ell,c}$ means in \eqref{eq:freq-sep}]
For a single shared carrier, \eqref{eq:freq-sep} is vacuous. By
Section~\ref{sec:carrier} the carrier is absent from the envelope
dynamics altogether, so there is no natural oscillation for the probe
to be confused with and $\omega_p$ is constrained only
by~\eqref{eq:adiabatic}. The condition becomes live again in exactly
two circumstances: under \emph{detuning}, where the residual
$\Delta\bm{\omega}$ of~\eqref{eq:detune-residual} survives the frame
change and $|\omega_p - \Delta\omega_{\ell,c}| > |\lambda_{\ell,c}|$
replaces it; and under multi-tone probing, where distinct
$\omega_p^{(k)}$ must be separated from one another and from all their
...
%======================================================================
\section{Rotating Substrate: Carrier Invariance}\label{sec:carrier}
%======================================================================

Everything so far treats $\bm{z}_\ell$ as a static relative phase. A
physical resonate-and-fire network does not hold still: each neuron's
instantaneous phase advances at a carrier $\omega$, and the information
lives in the phase \emph{differences}, which do. This section carries
out the extension deferred in Appendix~\ref{app:stationarity}: it shows
that for a single shared carrier the rotation cancels \emph{exactly},
in continuous and discrete time alike, so the estimator of
Section~\ref{sec:grad} needs no modification at all.

The result is stronger than an adiabatic approximation, and the
distinction matters operationally: an approximate cancellation would
demand a sweep to find the regime where it holds, whereas an exact one
means the static operating zone already \emph{is} the rotating one.

\subsection{The co-rotating substitution}

Write every layer's lab-frame state as a co-rotating envelope carried
by a common phase,
\begin{equation}\label{eq:corot-sub}
  \bm{z}_\ell(t) \;=\; e^{\im\omega t}\,\bm{w}_\ell(t),
  \qquad \ell = 0,1,\dots,L,
\end{equation}
applied to \emph{every} layer including the input $\bm{z}_0$. This is
the content of the per-channel $\omega$ rule: a single scalar $\omega$
shared by all channels and all layers, not a per-channel
$\bm{\omega}_\ell$. Substituting into the self-force variant of the
drift~\eqref{eq:settle-grad} with $K_\ell = \Lambda_\ell +
\im\omega I$, and using $\dot{\bm{z}}_\ell = e^{\im\omega
t}(\dot{\bm{w}}_\ell + \im\omega\bm{w}_\ell)$,
\begin{equation}\label{eq:carrier-cancel}
  \dot{\bm{w}}_\ell + \im\omega\bm{w}_\ell
  \;=\;
  (\Lambda_\ell + \im\omega)\bm{w}_\ell
  + W_\ell\bm{w}_{\ell-1}
  + W_{\ell+1}^{\!\top}\bm{w}_{\ell+1}
  \quad\Longrightarrow\quad
  \dot{\bm{w}}_\ell
  \;=\;
  \Lambda_\ell\bm{w}_\ell
  + W_\ell\bm{w}_{\ell-1}
  + W_{\ell+1}^{\!\top}\bm{w}_{\ell+1}.
\end{equation}
The $\im\omega$ generated by differentiating the carrier cancels the
$\im\omega$ in $K_\ell$ identically. What remains is the static drift
with $\Lambda_\ell$ in place of $K_\ell$ --- which, by
\eqref{eq:true-grad}, is exactly the Wirtinger gradient of the
energy. The rotating problem in the co-rotating frame \emph{is} the
static problem.

Two structural facts make this work, and both are worth isolating
because they are what fails when the model is perturbed.

\begin{proposition}[$U(1)$ invariance of the coupling]\label{prop:u1-inv}
For any $\theta\in\R$, the coupling term of~\eqref{eq:energy} is
invariant under the global rotation $\bm{z}_\ell \mapsto
e^{\im\theta}\bm{z}_\ell$ applied simultaneously to all layers
including $\bm{z}_0$:
\[
  \Real\langle W_\ell e^{\im\theta}\bm{z}_{\ell-1},\,
                e^{\im\theta}\bm{z}_\ell\rangle
  \;=\;
  \Real\bigl(e^{-\im\theta}e^{\im\theta}
      \langle W_\ell\bm{z}_{\ell-1},\bm{z}_\ell\rangle\bigr)
  \;=\;
  \Real\langle W_\ell\bm{z}_{\ell-1},\,\bm{z}_\ell\rangle .
\]
The self-energy is likewise invariant, being
$\tfrac12\sum_j\lambda_j|z_j|^2$. Both use only that $W_\ell$ is real
and the inner product Hermitian.
\end{proposition}

\begin{proposition}[$U(1)$ equivariance of the projector]
\label{prop:u1-equi}
$\mathrm{unit\_project}$ commutes with a global phase:
$\mathrm{unit\_project}(e^{\im\theta}g) = e^{\im\theta}g/|e^{\im\theta}g|
= e^{\im\theta}\,\mathrm{unit\_project}(g)$, since $|e^{\im\theta}g| =
|g|$.
\end{proposition}

Proposition~\ref{prop:u1-equi} is what lifts the cancellation from the
continuous flow to the implemented iteration, and it is not automatic:
it holds for $\mathrm{unit\_project}$ and for the smoothed variant
$g\mapsto g/\sqrt{|g|^2+\epsilon^2}$, but \emph{fails} for a projector
that interpolates phase toward a fixed reference, $g \mapsto
\exp(\im\,\sigma(|g|)\arg g)$, because $\sigma(|g|)\arg(e^{\im\theta}g)
\neq \theta + \sigma(|g|)\arg g$ unless $\sigma\equiv 1$. Such a
projector installs a preferred direction in the complex plane and
destroys everything in this section.

\subsection{Exactness in discrete time}

\begin{theorem}[Exact carrier reduction of the settling step]
\label{thm:carrier-exact}
Let $\bm{z}^{(n)}_\ell = e^{\im\omega n\dt}\bm{w}^{(n)}_\ell$ for all
$\ell$ including the input, and let the lab-frame iteration apply the
carrier as an exact rotation after the projection,
\begin{equation}\label{eq:lab-step}
  \bm{z}^{(n+1)}_\ell
  \;=\;
  e^{\im\omega\dt}
  \Bigl[(1-\dt)\,\bm{z}^{(n)}_\ell
        + \dt\;\mathrm{unit\_project}\bigl(\bm{g}^{(n)}_\ell\bigr)\Bigr].
\end{equation}
Then the envelopes $\bm{w}^{(n)}_\ell$ obey the static
step~\eqref{eq:settle-step} exactly, for every $\dt>0$ and every
$\omega$.
\end{theorem}

\begin{proof}
The drift is $U(1)$-covariant: each term of~\eqref{eq:settle-grad} is
linear in the states with real coefficients, so
$\bm{g}^{(n)}_\ell = e^{\im\omega n\dt}\,\bm{g}^{w,(n)}_\ell$, where
$\bm{g}^{w}$ is the drift evaluated on the envelopes. (The nudge and
bias require the co-rotating convention of
Section~\ref{sec:carrier-breaking}; the remaining terms are covariant
by inspection.) By Proposition~\ref{prop:u1-equi},
$\mathrm{unit\_project}(\bm{g}^{(n)}_\ell) = e^{\im\omega
n\dt}\,\mathrm{unit\_project}(\bm{g}^{w,(n)}_\ell)$. Substituting
into~\eqref{eq:lab-step},
\[
  e^{\im\omega (n+1)\dt}\bm{w}^{(n+1)}_\ell
  = e^{\im\omega\dt}\,e^{\im\omega n\dt}
    \Bigl[(1-\dt)\bm{w}^{(n)}_\ell
      + \dt\,\mathrm{unit\_project}(\bm{g}^{w,(n)}_\ell)\Bigr],
\]
and dividing by $e^{\im\omega(n+1)\dt}$ leaves
$\bm{w}^{(n+1)}_\ell = (1-\dt)\bm{w}^{(n)}_\ell +
\dt\,\mathrm{unit\_project}(\bm{g}^{w,(n)}_\ell)$.
\end{proof}

\begin{remark}[Multiplicative, not additive]
Theorem~\ref{thm:carrier-exact} requires the carrier to enter as the
\emph{multiplicative} factor $e^{\im\omega\dt}$, not as an additive
$\tfrac12\im\omega\bm{z}$ inside $\bm{g}_\ell$. The additive form
places the rotation inside the nonlinear projection, where it no longer
factors out, and it discretizes a rotation that the multiplicative form
represents exactly. The practical gap is large: with $\omega = 2\pi$
and $\dt = 0.5$ one has $\omega\dt = \pi$, exactly the Nyquist rate, at
which an additive step is meaningless while~\eqref{eq:lab-step} is
still exact. This is why the additive variant was reported to require
$\dt \ll 0.5$ merely to settle.
\end{remark}

\begin{remark}[Numerical asymmetry]
The two frames are equal in exact arithmetic but not in floating point.
The lab frame applies $e^{\im\omega\dt}$ once per step and accumulates
$O(N\cdot u)$ rounding over $N$ steps ($u$ the unit roundoff), plus
argument-reduction error in $e^{\im\omega t}$ once $\omega t$ grows
large; the co-rotating frame incurs neither. Measured agreement is
$10^{-7}$--$10^{-4}$ in relative state error, scaling with $N$ rather
than with $\omega$ or $\dt$, with directional agreement at
$\cos \geq 1-10^{-6}$. The co-rotating frame is therefore not merely
equivalent but numerically preferable; the lab frame's role is
verification.
\end{remark}

\subsection{The terms that break the symmetry}\label{sec:carrier-breaking}

Proposition~\ref{prop:u1-inv} covers the coupling and self-energy. It
does \emph{not} cover the bias or the cost, and both must be handled
explicitly:
\begin{equation}
  \Real\langle \bm{b}_\ell,\, e^{\im\theta}\bm{z}_\ell\rangle
  \;\neq\;
  \Real\langle \bm{b}_\ell,\, \bm{z}_\ell\rangle,
  \qquad
  \mathcal{C}(e^{\im\theta}\bm{z}_L,\bm{y})
  \;\neq\;
  \mathcal{C}(\bm{z}_L,\bm{y}).
\end{equation}
Left as lab-frame constants they would counter-rotate at $-\omega$ in
the envelope equation and, since $\omega \gg R_{\mathrm{relax}}$, would
time-average to approximately nothing --- silently deleting the bias
and the training signal.

The physical resolution is that in a spiking substrate neither is a
constant. A bias is delivered as a current pulse at a fixed phase of
every carrier cycle, so its fundamental Fourier component is
$\bm{b}\,e^{\im\omega t}$; a codebook is realized as neurons oscillating
at the same $\omega$, so the target is $\bm{y}\,e^{\im\omega t}$. Both
co-rotate, and with
\begin{equation}\label{eq:corot-breaking}
  \bm{b}_\ell(t) = e^{\im\omega t}\bm{b}_\ell,
  \qquad
  \bm{y}(t) = e^{\im\omega t}\bm{y},
\end{equation}
the substitution~\eqref{eq:corot-sub} removes the carrier from these
terms as well and~\eqref{eq:carrier-cancel} holds for the full drift.
Equivalently, and this is how it is implemented: evaluate the cost on
the \emph{demodulated} state and rotate the resulting force back,
\begin{equation}
  \bm{f}_{\mathrm{nudge}}(t)
  \;=\;
  e^{\im\omega t}\;
  \mathrm{nudge}\bigl(\mathcal{C},\;e^{-\im\omega t}\bm{z}_L(t),\;\beta\bigr),
\end{equation}
which is well-defined for any cost, including a softmax over codebook
similarities that could not simply be phase-rotated.

\subsection{Consequences for the estimator}

\begin{corollary}[The estimator is unchanged]\label{cor:est-unchanged}
Under~\eqref{eq:corot-sub} and~\eqref{eq:corot-breaking}, the lock-in
gradient of Theorem~\ref{thm:lockin-grad} computed on the envelopes
$\bm{w}_\ell$ is identical to the static-substrate estimate. In
particular the demodulation frequency remains $\omega_p$, the
integer-cycle condition of Section~\ref{sec:knobs} is unchanged, and
the adiabaticity requirement is still $\omega_p \ll R_{\mathrm{relax}}$
with no reference to $\omega$.
\end{corollary}

In the lab frame the same content appears as sidebands. The probe
multiplies a carrier, $\varepsilon\cos(\omega_p t)\,e^{\im\omega t} =
\tfrac{\varepsilon}{2}\bigl(e^{\im(\omega+\omega_p)t} +
e^{\im(\omega-\omega_p)t}\bigr)$, so the linear response sits at
$\omega\pm\omega_p$ and a lab-frame demodulator must accumulate both
and \emph{sum} them --- each carries $\varepsilon/2$, so the sum
restores the amplitude that the $-2/(T\varepsilon)$ prefactor of
Theorem~\ref{thm:lockin-grad} already assumes. Averaging the two
sidebands instead of summing them introduces a spurious factor of
$\tfrac12$. A further trap: with a carrier present, the integer-cycle
condition that nulls the DC term at $\omega_p$ no longer suffices, as
carrier leakage $\sum_t e^{\im(\omega-\omega_p)t\dt}$ must also vanish.
Both complications are artifacts of working in the lab frame; in the
co-rotating frame there is one sideband, at $\omega_p$, and neither
arises.

\subsection{The Hebbian is a relative phase}

The reason this program can run on a spiking substrate at all is
visible in the Hebbian. On the torus,
\begin{equation}\label{eq:hebb-relative}
  \Real\bigl(z_{\ell,i}\,\bar z_{\ell-1,j}\bigr)
  \;=\;
  \cos\bigl(\arg z_{\ell,i} - \arg z_{\ell-1,j}\bigr),
\end{equation}
a function of the phase \emph{difference} alone. It is therefore
$U(1)$-invariant --- the carrier cancels between the two factors
without any demodulation --- and it is precisely what a synapse can
measure from the interval between a presynaptic and a postsynaptic
spike. The conjugation is load-bearing: replacing $\bar z_{\ell-1,j}$
with $z_{\ell-1,j}$ gives $\cos(\arg z_{\ell,i} + \arg z_{\ell-1,j})$,
which acquires $e^{2\im\theta}$ under a global rotation, is not a
relative phase, and is not measurable from spike timing. (The
holomorphic companion derivation uses the transposed form deliberately,
for a Wirtinger derivative in a different convention; it does not carry
over here.)

\subsection{Boundary of validity: the readout}\label{sec:carrier-readout}

Theorem~\ref{thm:carrier-exact} is a statement about the \emph{state}.
It says nothing about how the state is observed, and the distinction is
not idle: a spiking substrate does not report $\bm{z}_\ell$, it reports
a spike time quantized against a clock.

Model that as $Q_\delta$, rounding the phase onto a grid of spacing
$\delta$ turns,
\begin{equation}\label{eq:quantizer}
  Q_\delta(z) \;=\; |z|\,
  \exp\!\Big(2\pi\im\,\delta\,
  \operatorname{round}\!\big(\tfrac{\arg z}{2\pi\delta}\big)\Big).
\end{equation}
Every other operation in the settle is $U(1)$-covariant or
$U(1)$-equivariant (Propositions~\ref{prop:u1-inv}
and~\ref{prop:u1-equi}), which is what makes the carrier
cancel. $Q_\delta$ is neither. It satisfies
\begin{equation}\label{eq:quantizer-equivariance}
  Q_\delta(e^{2\pi\im\theta}z) \;=\; e^{2\pi\im\theta}Q_\delta(z)
  \qquad\text{if and only if}\qquad \theta/\delta \in \mathbb{Z},
\end{equation}
so it commutes with the carrier only on the subgroup
$\{e^{2\pi\im k\delta}\}\subset U(1)$ generated by the grid.

\begin{remark}[The frame becomes a modelling choice]
Consequently the carrier reduction extends to a quantized pipeline only
when the sampling advance is commensurate with the grid: writing
$\Delta t_{\mathrm{s}}$ for the interval between observations, the
condition is $\omega\,\Delta t_{\mathrm{s}}/2\pi \in \delta\mathbb{Z}$.
That holds automatically when the readout is one spike time per carrier
period, since the advance is then exactly one turn. It fails generically
for a clock that free-runs against the carrier, and in that case the
carrier sweeps the state across many bins between samples and acts as a
deterministic dither --- so the co-rotating and lab formulations are
genuinely different estimators, and the co-rotating one is the
pessimistic member of the pair. Where the readout clock is locked is
therefore a physical assumption that must be stated, not a choice of
coordinates. Measurements in \texttt{docs/ep\_rotating\_extension.md}.
\end{remark}

\subsection{Boundary of validity: non-uniform \texorpdfstring{$\bm{\omega}$}{omega}}

Everything above needs one shared $\omega$. With per-channel carriers
$\omega_{\ell,c} = \bar\omega + \Delta\omega_{\ell,c}$, the
substitution at $\bar\omega$ leaves a residual
\begin{equation}\label{eq:detune-residual}
  \dot{\bm{w}}_\ell
  \;=\;
  (\Lambda_\ell + \im\,\Delta\bm{\omega}_\ell)\bm{w}_\ell
  + W_\ell\bm{w}_{\ell-1}
  + W_{\ell+1}^{\!\top}\bm{w}_{\ell+1},
\end{equation}
and $\im\Delta\bm{\omega}_\ell\odot\bm{w}_\ell$ is tangential to the
torus and non-variational, exactly the obstruction of
Appendix~\ref{app:stationarity}. No global frame removes it: the system
has a relative equilibrium only if the couplings phase-lock the
detuned channels, which is an Adler/Kuramoto locking condition rather
than an adiabaticity condition. The relevant dimensionless group is
$\Delta\omega / R_{\mathrm{relax}}$, and above threshold the ``equilibrium''
is a torus rather than a point --- a regime in which a fidelity metric
evaluated on a snapshot reports noise, and should be read alongside a
stationarity residual to avoid mistaking drift for estimator error.
This is the one direction in which a rotating substrate genuinely
requires new theory, and it is left open.
...
\item On the projected (torus-constrained) equilibrium, the EP
  ...
  requires the rotating-frame reduction before the theorem applies.
  For a single shared carrier that reduction is \emph{exact}, in
  discrete time as well as continuous, so the estimator and its
  operating zone carry over unchanged
  (Section~\ref{sec:carrier}); for non-uniform $\bm{\omega}$ it fails
  and the open question becomes a locking threshold rather than an
  adiabaticity limit.
...
construction that extends the derivation to the self-force variant is
carried out in Section~\ref{sec:carrier}, where the cancellation is
shown to be exact in discrete time as well
(Theorem~\ref{thm:carrier-exact}) provided the carrier is applied
multiplicatively and the bias and target co-rotate.
