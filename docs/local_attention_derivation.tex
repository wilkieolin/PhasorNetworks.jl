...
\begin{align}
  s_{a h}^{(l,b)}
    &= \sm\!\bigl(Q^{*}_{:,a,h},\, K_{:,h,l,b}\bigr)
       ...
       \label{eq:lca-score} \\
  \tilde s_{a h}^{(l,b)}
    &= \frac{1}{A}\exp\!\bigl(\beta\,s_{a h}^{(l,b)}\bigr),
       \label{eq:lca-scale} \\
  R_{:,h,l,b}
    &= \sum_{a=1}^{A} \tilde s_{a h}^{(l,b)}\,
       e^{\im\pi Q^{*}_{:,a,h}}
       \in \C^{\Dh},
       \label{eq:lca-retrieve} \\
  \tilde Y_{:,h,l,b}
    &= e^{\im\pi V_{:,h,l,b}} \odot R_{:,h,l,b}
       \in \C^{\Dh},
       \label{eq:lca-mix} \\
  Y_{:,l,b}
    &= \texttt{reshape}_{(\Dh,\nh)\to D}\!\bigl(
       ...
       \label{eq:lca-reduce}
\end{align}
where $\odot$ denotes element-wise complex multiplication (Hadamard
product) and the retrieved memory $R_{:,h,l,b}$ is a
similarity-weighted bundle of the per-head anchors.
\end{definition}

\begin{remark}[Why bind, instead of bundle V directly?]\label{rem:lca-binding}
A naive variant of~\eqref{eq:lca-mix} would bundle
$V_{:,h,l,b}$ directly, weighted by the per-anchor scores
$\tilde s_{a h}^{(l,b)}$:
\(
  \tilde Y^{\mathrm{naive}}_{:,h,l,b}
  = \bigl(\sum_a \tilde s_{a h}^{(l,b)}\bigr)\cdot e^{\im\pi V_{:,h,l,b}}.
\)
Since the parenthesised factor is a real positive scalar, this assignment
leaves $\arg \tilde Y^{\mathrm{naive}}_{:,h,l,b} =
\pi\,V_{:,h,l,b}$ unchanged --- the attention scores affect only
amplitude, and the layer reduces to identity in phase. The binding form
\eqref{eq:lca-mix} repairs the degeneracy by retrieving a phasor (the
anchor bundle~$R$) and binding it onto~$V$, so the per-anchor scores
shape the output's phase.
\end{remark}
...
\begin{remark}[Optional trainable head-mix]\label{rem:lca-trainable-mix}
A natural extension promotes the per-head bundle
of~\eqref{eq:lca-retrieve} to a learned cross-head mix:
\(
  R^{(M)}_{:,h,l,b}
  = \sum_{a, h'} M_{a h h'}\,\tilde s_{a h'}^{(l,b)}\,e^{\im\pi Q^{*}_{:,a,h'}},
\)
with $M\in\R^{A\times \nh\times \nh}$ trainable. The default $M_{a h h'}
= \delta_{h h'}/A$ recovers~\eqref{eq:lca-retrieve}. The added cost is
$A\,\nh^2$ parameters and a $\Dh\,A\,\nh^2\,L\,B$ contraction. We leave
this ablation to future work.
\end{remark}


\subsection{LCA as a modern Hopfield retrieval (with input binding)}
...
the Euclidean inner product. The retrieval~\eqref{eq:lca-retrieve}
returns the softmax-weighted anchor bundle $R_{:,h,l,b}$; the
mix~\eqref{eq:lca-mix} binds that bundle into the input value
$V_{:,h,l,b}$ via element-wise complex multiplication.

\begin{proposition}[Anchors as stored patterns under input binding]\label{prop:lca-hopfield}
Define the per-head pattern matrix $\Xi_h \in \C^{\Dh \times A}$ with
columns $\Xi_{:,a,h} = e^{\im\pi Q^{*}_{:,a,h}}$, and let
$\mathrm{HopRet}_h(K) \in \C^{\Dh}$ denote the modern-Hopfield retrieval
of~\cite{hopfield-is-all-you-need} applied per head with stored patterns
$\Xi_h$, query $e^{\im\pi K_{:,h,l,b}}$, inverse temperature $\beta$,
and the Fourier-HRR inner product. Then
\[
  \tilde Y_{:,h,l,b}
  = e^{\im\pi V_{:,h,l,b}} \odot \mathrm{HopRet}_h(K),
\]
\ie, LCA realises a Hopfield retrieval whose result is bound onto the
input value $V$. Equivalently, the per-channel output phase decomposes
as
\[
  Y_{:,h,l,b}
  = V_{:,h,l,b}
  \;\oplus\; \tfrac{1}{\pi}\arg \mathrm{HopRet}_h(K),
\]
where $\oplus$ denotes phase addition $\bmod\,2$ on $[-1,1]$
(\cite{phasor-ssm-derivation}, eq.~(17)).
\end{proposition}

\begin{proof}
Substitute the Fourier-HRR similarity~\eqref{eq:lca-score} into
\eqref{eq:lca-scale}. The real part of $\Xi_h^{\dagger}\,
e^{\im\pi K_{:,h,l,b}}$ equals $\Dh \cdot
\sm(Q^{*}_{:,a,h}, K_{:,h,l,b})$ component-wise, so the per-anchor
exponential reproduces $\tilde s_{a h}^{(l,b)}$. The bundle
\eqref{eq:lca-retrieve} is then exactly the column-contraction
$\Xi_h \cdot \tilde s_{:,h}^{(l,b)}$, which is the modern-Hopfield
retrieval rule of~\cite{hopfield-is-all-you-need}. The binding step
\eqref{eq:lca-mix} multiplies elementwise by $e^{\im\pi V_{:,h,l,b}}$;
under the unit-torus convention this is phase addition.
\end{proof}

\begin{remark}[Limit cases]\label{rem:lca-hopfield-limits}
Two special cases are worth noting:
\begin{itemize}[nosep,leftmargin=*]
  \item $V \equiv 0$ (identity input value): the binding factor is
        unity, and LCA reduces to pure modern-Hopfield retrieval ---
        the output phase is $\tfrac{1}{\pi}\arg \mathrm{HopRet}_h(K)$.
  \item $\beta\to\infty$ (winner-take-all): $\tilde s_{a^{\star} h}\to 1/A$
        for the closest anchor $a^{\star}$ and $\to 0$ elsewhere, so
        $R_{:,h,l,b} \to A^{-1}\,e^{\im\pi Q^{*}_{:,a^{\star}, h}}$
        and the output phase becomes $V_{:,h,l,b}$ bound with the
        winning anchor's phase.
\end{itemize}
\end{remark}
...
that the inner bundle $R_{:,h,l,b}$ of LCA realises this update with
stored patterns $\Xi_h$ on the unit torus and the Fourier-HRR inner
product as similarity; LCA then binds the retrieved memory onto the
input value $V$, recovering the pure Hopfield-retrieval rule in the
limit $V \equiv 0$. The unit-modulus restriction is the same one used
throughout \cite{phasor-ssm-derivation}, \S4, and preserves Plate's
binding/bundling algebra~\cite{hrr}.
...
$\Pi_K, \Pi_V$ of width $D$, a scale $\beta\in\R_+$, and the carrier
...
every lattice point $l T$ (Proposition~\ref{prop:lca-equiv}). It
realises a modern-Hopfield retrieval on the unit torus with stored
patterns $\Xi_h$, whose output is bound onto the input value $V$
(Proposition~\ref{prop:lca-hopfield}); the pure Hopfield-retrieval
limit is recovered when $V \equiv 0$. An optional trainable head-mix
$M$ generalises~\eqref{eq:lca-retrieve}; see
Remark~\ref{rem:lca-trainable-mix}.
\end{definition}
